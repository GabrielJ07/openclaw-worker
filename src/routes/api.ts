...
adminApi.post('/devices/approve-all', async (c) => {
  ...
  try {
    ...
    let proc = await findExistingGatewayProcess(sandbox);
    if (!proc) {
      // Port probe as backup
      const isPortOpen = await sandbox.exec(`nc -z localhost 18789`).then(r => r.exitCode === 0).catch(() => false);
      if (!isPortOpen) {
        // Not running, try to start it
        proc = await ensureGateway(sandbox, c.env);
        if (!proc) {
          // Still not running or starting
          return c.json({ error: 'Gateway not running and failed to start' }, 503);
        }
      }
    }
    const token = c.env.OPENCLAW_GATEWAY_TOKEN || c.env.MOLTBOT_GATEWAY_TOKEN;
