...
publicRoutes.get('/api/status', async (c) => {
  ...
  try {
    ...
    // Try to reach the gateway with a 12s timeout. Gateway starts in ~10-15s.
    // Setting this longer avoids the 'not_responding' flip-flop in the UI.
    try {
      await process.waitForPort(GATEWAY_PORT, { mode: 'tcp', timeout: 12000 });
      return c.json({ ok: true, status: 'running', processId: process.id });
    } catch {
      // Still booting or slow to respond
      return c.json({ ok: false, status: 'starting', processId: process.id, note: 'waiting_for_port' });
    }
  } catch (err) {
    return c.json({
      ok: false,
