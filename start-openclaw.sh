...
if [ ! -f "$CONFIG_FILE" ]; then
    ...
    if [ -n "$CLOUDFLARE_AI_GATEWAY_API_KEY" ] && [ -n "$CF_AI_GATEWAY_ACCOUNT_ID" ] && [ -n "$CF_AI_GATEWAY_GATEWAY_ID" ]; then
        ...
        AUTH_ARGS="--auth-choice gemini-api-key"
...
// Default to Gemini 3.1 Flash if GEMINI_API_KEY is present and no model is set yet
if (process.env.GEMINI_API_KEY && (!config.agents?.defaults?.model || process.env.FORCE_GEMINI === 'true')) {
    config.agents = config.agents || {};
    config.agents.defaults = config.agents.defaults || {};
    config.agents.defaults.model = { primary: 'gemini/gemini-3.1-flash' };
    console.log('Setting default model to Gemini 3.1 Flash');
}
